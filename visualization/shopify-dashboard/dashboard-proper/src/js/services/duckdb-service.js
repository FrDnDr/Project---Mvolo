...
function parseProfitabilityFileName(fileName) {
  // Pattern 1: Shopify (shopify_week_01_2026-01-01_to_2026-01-07.csv)
  let match = /^shopify_week_(\d{1,2})_(\d{4}-\d{2}-\d{2})_to_(\d{4}-\d{2}-\d{2})\.csv$/i.exec(fileName);
  if (match) {
    return {
      fileName,
  ...
  // Pattern 2: Bol (profitability_2026-01-01_to_2026-01-07_Week01.csv)
  match = /^profitability_(\d{4}-\d{2}-\d{2})_to_(\d{4}-\d{2}-\d{2})_Week_?(\d{1,2})\.csv$/i.exec(fileName);
  if (match) {
    return {
      fileName,
      start: match[1],
